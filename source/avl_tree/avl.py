...
class avl_tree:
    ...
    def insert(self, val, index, root):
        if root is None:
            return node(val, index)
        elif val <= root.value:
            root.left = self.insert(val, index,root.left)
        elif val > root.value:
            root.right = self.insert(val, index,root.right)
    ...
    def get_hightest_value(self, root):
        if root.right is None:
            print(root.value)
            return root.value
            
        return self.get_hightest_value(root.right)
